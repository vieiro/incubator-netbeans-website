...
Software or ResourceVersion Requiredlink:http://download.netbeans.org/[+NetBeans IDE+]version 7.1link:http://java.sun.com/javase/downloads/index.jsp[+Java Developer Kit (JDK)+]version 6 or above
...
[start=3]
3. Type *MavenPlatformWordApp* for the Project Name and set the Project Location.
...
[start=2]
2. In the Basic tab, modify the Application Title to *My Maven Platform Word App*.image::images/maven-branding1.png[title="Screenshot of New Project wizard"]
...
[start=3]
3. Type *MavenWordEngine* as the Project Name:

image::images/new-module-3.png[title="Screenshot of New Project wizard"]


[start=4]
4. Click Browse and locate the MavenPlatformWordApp directory as the Project Location.

image::images/new-module-2.png[title="Screenshot of New Project wizard"]


[start=5]
5. Click Next and select the NetBeans APIs you want to use:
...
[start=2]
2. Click the Open Projects tab and select *MavenWordEngine*. Click OK.
...
[start=2]
2. Select *output* in the Window Position dropdown list. Click Next.image::images/maven-new-window.png[title="Screenshot of window component page in New File wizard"]

[start=3]
3. Type *Text* in the Class Name Prefix field. Click Finish.
...
[start=2]
2. Drag and drop a button and a text area from the Palette (Ctrl-Shift-8) onto the window.
...
[start=5]
5. Double-click the Filter! button in the Design view to open the event handler method for the button in the source code editor. The method is created automatically when you double-click the button element and the Source view opens. Modify the body of the method to add the following code.
...
[start=3]
3. Type *TextFilter* for the Project Name. Click Browse to set the Project Location and locate the application directory:
...
[start=2]
2. As explained in the previous section, select NetBeans Module from the Maven category. Click Next.

[start=3]
3. Type *MyFilter* for the Project Name.

[start=4]
4. Click Browse to set the Project Location and locate the application directory. Click Finish.

[start=5]
5. Add the MyFilter module as a dependency of the *application* project.
...
[start=2]
2. Expand the Source Packages of the *MavenWordEngine* module and open  ``TextTopComponent``  in the source editor.

[start=3]
3. Modify the  ``jButton1ActionPerformed``  button handler method to add the following code. Save your changes.
...
[start=2]
2. Select NetBeans Module from the Maven category. Click Next.
...
[start=5]
5. Add the History module as a dependency of the *application* project.
...
[start=2]
2. Select *editor* in the Window Position dropdown list. Click Next.

[start=3]
3. Type *History* in the Class Name Prefix field. Click Finish.
...
[start=4]
4. Implement the  ``LookupListener``  class and add the following code to the  ``HistoryTopComponent``  so that it listens to the Lookup for the String class whenever the  ``HistoryTopComponent``  opens and displays all retrieved String objects in the text area.
...
[start=2]
2. Right-click the project node of the *application* and choose Build with Dependencies.

[start=3]
3. Right-click the project node of the *application* and choose Run.
...
This tutorial demonstrated the fundamental pieces of NetBeans Platform applications. Creating a NetBeans Platform application using Maven is not very different from creating an application using Ant. The major difference is understanding how the Maven POM controls how the application is assembled. For more examples on how to build NetBeans Platform applications and modules, see the tutorials listed in the link:https://netbeans.org/kb/trails/platform.html[+NetBeans Platform Learning Trail+].
 ...
 link:https://netbeans.org/about/contact_form.html?to=3&subject=Feedback:%20NetBeans%20Platform%20Quick%20Start%207.1%20Using%20Maven[+ Send Us Your Feedback+]
