...
Software or ResourceVersion Requiredlink:https://netbeans.org/downloads/index.html[+NetBeans IDE+]version 7.1link:http://java.sun.com/javase/downloads/index.jsp[+Java Developer Kit (JDK)+]version 6 or above
...
[start=2]
2. The module is built and installed in a new instance of the IDE. Open an HTML file or an XML file, double-click on a word, and notice that all matching words are automatically highlighted.

[start=3]
3. Navigate from one matching word to the next via Ctrl-F3.
link:https://netbeans.org/about/contact_form.html?to=3&subject=Feedback:%20Mark%20Occurrences%20Module%207.1%20Tutorial[+Send Us Your Feedback+]
