...
ソフトウェアまたはリソース必須バージョンlink:http://download.netbeans.org/netbeans/6.9/beta/[+NetBeans IDE+]version 6.9 (Java)link:http://java.sun.com/javase/downloads/index.jsp[+Java Developer Kit (JDK)+]version 6 or abovelink:http://maven.apache.org/[+Maven+]version 2.0.9 以降JavaDB またはほかのデータベースサーバーおよびデータベース 
