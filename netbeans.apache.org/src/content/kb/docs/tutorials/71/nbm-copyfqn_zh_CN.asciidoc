...
﻿platform: 针对 NetBeans 平台 6.5 的 NetBeans Java 语言基础结构教程         
...
由 link:http://zh-cn.netbeans.org/community[+NetBeans 中文社区+]的 link:mailto:guanminglin@gmail.com[+日月雨林+] 翻译（2009年 8 月）

参考link:https://platform.netbeans.org/tutorials/60/nbm-copyfqn_zh_CN.html[+NetBeans 6.0 版本的翻译+]

*提示: *这份文档介绍的是基于 NetBeans IDE 6.5 . 如果你使用的是更早的版本, 请看link:60/nbm-copyfqn.html[+Netbeans 6.0/6.1 版本的文档+].
...
软件或资源要求的版本link:https://netbeans.org/downloads/index.html[+NetBeans IDE+]版本 6.5link:http://java.sun.com/javase/downloads/index.jsp[+Java Developer Kit (JDK)+]版本 6 or版本 5
...
[start=2]
2. 在“项目名称”中键入  ``CopyFQN`` ，将“项目位置”设置为硬盘上的相应文件夹。选择“独立模块”和“设置为主项目”（如果未选择这两个选项）。单击“下一步”。

[start=3]
3. 在“代码名称基”中键入  ``org.netbeans.modules.copyfqn`` ，在“模块显示名称”中键入  ``CopyFQN`` 。单击“完成”。

[start=4]
4. 右键单击项目，选择“属性”，单击“项目属性”对话框中的“库”，然后声明与以下 API 的依赖关系：
...
[start=4]
4. 在“类名”中键入  ``CopyFQNAction`` ，在“显示名称”中键入  ``CopyFQN`` 。

[start=5]
5. 浏览到将显示在此工具栏按钮中的图标。例如，使用将在本教程中使用的图标：
...
*提示:* 们在本教程其余部分执行的所有工作将集中于上面的  ``performAction()``  方法。
...
[start=7]
7. 在“项目”窗口中选择一个节点，然后在工具栏中查看此按钮。如果选择一个表示文件或文件夹（包括包）的节点，此按钮将启用，如下所示：
...
在本节中，我们开始使用一个新的 "Retouche" API，此 API 称作link:https://netbeans.org/download/dev/javadoc/org-netbeans-modules-java-source/overview-summary.html[+Java Source+]. 在此，我们使用link:https://netbeans.org/download/dev/javadoc/org-netbeans-modules-java-source/org/netbeans/api/java/source/JavaSource.html[+JavaSource+] class, 类，它表示一个 Java 源文件。我们为与数据对象关联的文件对象返回此类的一个实例。如果返回 null，则说明此文件对象不是 Java 源文件。在选定某个文件的情况下单击此按钮时，将在状态栏中显示结果。
...
[start=2]
2. 单击此灯泡。或者，在该行中放入插入记号，再按 Alt-Enter 组合键。然后让 IDE 实现此方法。

[start=3]
3. 稍微调整一下此方法，方法是：在方法的末尾添加一个  ``true``  布尔值，并让 IDE 将此代码片段包装在一个 try/catch 块中。最后，结果应如下所示：
...
[start=2]
2. 将  ``run()``  方法中的 "Hurray!" 消息替换为下面高亮显示的代码：
...
[start=3]
3. 以下是  ``MemberVisitor``  类，将其定义为  ``CopyFQNAction``  类的内部类：
...
[start=4]
4. 再次安装此模块，并打开一个 Java 类。然后单击此按钮，并注意构造函数、方法以及字段已写入“输出”窗口中，如下所示：

image::images/output-window-60.png[]


[start=5]
5. 接下来，不在“输出”窗口中显示所有元素，而仅显示插入记号下的元素。只需将  ``visitClass``  方法替换为下面高亮显示的代码：
...
[start=7]
7. 将插入记号放在 Java 代码中的某处，然后按此按钮。“输出”窗口显示有关插入记号下的代码的信息（如果适用）。例如，如果将插入记号放入某个方法中后按此按钮，“输出”窗口将通知您插入记号位于方法中，如下所示：

image::images/message-constructor-60.png[]


[start=8]
8. B但是，除了检测到插入记号下的元素的名称外，还可以检测到其他许多信息。在  ``visitClass``  方法中，替换下面以粗体显示的代码行：
...
[start=9]
9. 再次安装此模块。这一次，如果您在插入记号位于某个构造函数、方法或字段上时单击此按钮，将在“输出”窗口中显示有关此元素的更多详细信息。
...
[start=2]
2. 接下来，将代码中的每个 "Hurray!" 行替换为这样一行代码，该代码行将此元素作为一个字符串发送到将在下一步定义的方法。我们将此方法称为  ``setClipboardContents`` 。因此，将第一个 "Hurray!" 行替换为下面的代码行：
...
[start=3]
3. 最后，将以下代码添加到类的末尾。此方法接收字符串并将它放入剪贴板中：
...
link:https://netbeans.org/about/contact_form.html?to=3&subject=Feedback:%20Java%20Language%20Infrastructure%20Tutorial[+请将您的意见和建议发送给我们+]
