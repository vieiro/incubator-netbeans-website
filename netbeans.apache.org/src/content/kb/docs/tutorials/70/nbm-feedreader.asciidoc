...
Software or ResourceVersion Requiredlink:https://netbeans.org/downloads/index.html[+NetBeans IDE+]version 7.0link:http://java.sun.com/javase/downloads/index.jsp[+Java Developer Kit (JDK)+]Version 6link:http://repo1.maven.org/maven2/net/java/dev/rome/rome/1.0.0[+Rss and atOM utilitiEs+]link:http://repo1.maven.org/maven2/net/java/dev/rome/rome-fetcher/1.0.0[+Rome Fetcher+]link:http://jdom.org/downloads/index.html[+JDom+]link:https://netbeans.org/files/documents/4/550/feedreader-images.zip[+FeedReader icon and splash screen+]
...
[start=4]
4. Select "Generate XML Layer", but do not select "Generate OSGi Bundle". Leave the location of the XML layer file so that it will be stored in the main package, which has the name  ``org/myorg/feedreader`` . Click Finish.
...
[start=3]
3. Type  ``RssFeeds``  in the New Folder dialog box. Click OK.
...
link:https://netbeans.org/about/contact_form.html?to=3&subject=Feedback: NetBeans Platform 7.0 Feed Reader Tutorial[+Send Us Your Feedback+] 
