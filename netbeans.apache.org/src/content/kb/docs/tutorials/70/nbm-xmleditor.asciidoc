...
[start=2]
2. In the Name and Location panel, type  ``ShowXMLStructure``  in the Project Name field. Change the Project Location to any directory on your computer. Select the Standalone Module option and Set as Main Project checkbox selected. Click Next.
...
[start=5]
5. In the Source Editor, write the  ``actionPerformed``  method as follows, after reading and understanding the comments in the code:
...
[start=7]
7. Add the display names to the  ``Bundle.properties``  file:
