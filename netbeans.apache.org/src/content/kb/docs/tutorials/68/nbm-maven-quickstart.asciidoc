...
Software or ResourceVersion Requiredlink:https://netbeans.org/downloads/index.html[+NetBeans IDE+]version 6.8link:http://java.sun.com/javase/downloads/index.jsp[+Java Developer Kit (JDK)+]Version 6 orversion 5link:http://maven.apache.org/[+Maven+]Version 2.0.9 or higherlink:http://subversion.tigris.org/[+Subversion+]Version 1.6.x
...
For details on setting up Subversion, see the link:https://netbeans.org/kb/docs/ide/subversion.html[+Guided Tour of Subversion+].
...
[start=2]
2. Select Maven Project from the Maven category. Click Next.

[start=3]
3. Select Maven NetBeans Platform Application Archetype. Click Next.

[start=4]
4. Type *MavenPlatformWordApp* for the Project Name and set the Project Location. Click Finish. image::images/maven-nbm-newprojectwizard1.png[title="Screenshot of New Project wizard"]
...
[start=6]
6. Modify the body of the method to add the following code and save your changes.
...
[start=1]
1. Choose Window > Text from the main menu of the platform application.image::images/maven-nbm-wordapp.png[title="Screenshot of Text window in application"]

[start=2]
2. Type some characters in the text area and click Filter!
...
[start=2]
2. Expand the Project Files node and double-click  ``pom.xml``  to open the POM in the editor.

[start=3]
3. Modify the  ``<netbeans.version>``  element to change the version to  ``RELEASE68``  and save your changes.
...
[start=3]
3. Modify the  ``<artifactId>``  element to change the version to  ``platform11``  and save your changes.
...
[start=2]
2. Replace the default image with an image named  ``splash.gif`` .
