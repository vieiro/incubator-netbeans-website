...
[start=4]
4. Selecione "Gerar camada XML". Deixe as localizações do pacote de localizações e a camada XML, de modo que sejam armazenados em um pacote com o nome  ``org/demo/wizard`` . Clique em Terminar.
...
O elemento "iconBase" aponta para uma imagem denominada "icon.png" do pacote principal. Use sua própria imagem com tal nome, certificando-se de que o tamanho seja de 16x16 pixels ou use a imagem seguinte: image::images/icon.png[]


[start=3]
3. Execute o módulo. O aplicativo se inicia e o botão deveria ser visto na barra de ferramentas que você especificou no arquivo  ``layer.xml`` :
...
[start=3]
3. Abra os arquivos  ``DemoVisualPanel1.java``  e  ``DemoVisualPanel2.java``  e use o construtor de GUI "Matisse" para adicionar alguns componentes Swing, como os seguintes:
...
[start=2]
2. Depois, use o arquivo  ``DemoWizardAction.java``  para recuperar as propriedades que você definiu e adote o seguinte procedimento com tais propriedades:
...
[start=4]
4. Execute o módulo. Clique em "Next", sem introduzir nada no campo "Name" e você deveria ver o resultado abaixo. Observe também que não é possível mover para o painel seguinte, como conseqüência da falha na validação:

image::images/validation1.png[]


[start=5]
5. Como opção, desabilite o botão "Next" se o campo do nome estiver vazio. Comece declarando um booleano na parte superior da classe:
...
[start=4]
4. Execute o aplicativo novamente e, da próxima vez que o assistente for aberto, as configurações especificadas acima serão automaticamente usadas para definir os valores nos campos do assistente.
