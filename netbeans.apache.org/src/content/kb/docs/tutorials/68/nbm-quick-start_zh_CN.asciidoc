...
[start=8]
8. 双击该按钮，将在源代码编辑器中自动创建一个事件处理方法。在单击按钮时，将会调用此方法。将此方法的主体更改为以下代码。
...
[start=5]
5. 向 HistoryTopComponent 类的构造函数中添加一些代码，使其侦听当前活动窗口  ``String``  类的 lookup 事件。该代码会在文本区域中显示检索到的所有  ``String``  对象：
