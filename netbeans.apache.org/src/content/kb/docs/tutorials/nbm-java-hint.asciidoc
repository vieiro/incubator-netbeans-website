...
For troubleshooting purposes, you are welcome to download the link:http://java.net/projects/nb-api-samples/sources/api-samples/show/versions/8.0/tutorials/MyCustomHints[+completed tutorial source code+].
...
[start=1]
1. Use Mercurial to clone the NetBeans sources: link:http://wiki.netbeans.org/DevFaqAccessSourcesUsingMercurial[+http://wiki.netbeans.org/DevFaqAccessSourcesUsingMercurial+]
...
[start=2]
2. In the Name and Location panel, type  ``MyCustomHints``  in the Project Name field. Change the Project Location to any directory on your computer. Click Next.

[start=3]
3. In the Basic Module Configuration panel, type  ``org.my.custom.hints``  in Code Name Base. Click Finish.
...
[start=1]
1. Right-click the project node and choose New > Other > Module Development > Java Hint: image::images/new-hint-1.png[]

Click Next.


[start=2]
2. In the New Java Hint Description panel, fill out the fields as follows: image::images/new-hint-2.png[]

Click Next.


[start=3]
3. In the New Java Hint Location panel, fill out the fields as follows: image::images/new-hint-3.png[]
...
[start=4]
4. Replace the  ``@TriggerPattern``  with the following:
...
[start=5]
5. Right-click the module and choose Build. Open the Files window (Ctrl-2) and browse to the generated layer file, which is shown below:
...
[start=2]
2. Add the fix to the error description you defined in the previous section; you only need to add the code highlighted below:
...
[start=3]
3. Check that you have these import statements:
...
[start=4]
4. Install the module again and you will be able to click the hint to delete the superfluous statement:
...
link:https://netbeans.org/about/contact_form.html?to=3&subject=Feedback:%20Java%20Hint%208.0%20Tutorial[+Send Us Your Feedback+]
...
* link:https://platform.netbeans.org/tutorials/nbm-copyfqn.html[+NetBeans Java Language Infrastructure Tutorial+]
