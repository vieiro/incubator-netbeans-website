...
* *SJDataObject.java.* Represents one or more file types, letting you parse and process the underlying file content. For more information, see link:http://wiki.netbeans.org/wiki/view/DevFaqDataObject[+What is a DataObject?+].
...
*Note:* Each  ``<file>``  under  ``<folder name="Actions">``  associates the files of the  ``text/x-sj``  mime type with one of the predefined actions provided by the NetBeans Platform. A " ``.shadow`` " file means that the action specification is actually somewhere else in the NetBeans virtual filesystem, as specified by the  ``originalFile``  attribute. To add more actions for SJ files, see the link:https://platform.netbeans.org/tutorials/nbm-filetype.html[+File Type Integration Tutorial+].


[start=5]
5. Right-click the module and choose Run. The application starts up, installing your module. Go to the Favorites window (Ctrl-3) and open a few files on computer that have ".sj" as their file extension.
...
[start=7]
7. The  ``Java1.5.jj``  file is ready now and we can "compile" it from the command line:
...
[start=5]
5. Register the  ``FontAndColors.xml``  file, as well as the example file into the  ``layer.xml``  file:
...
[start=6]
6. Check that your module now has this content:
...
link:https://netbeans.org/about/contact_form.html?to=3&subject=Feedback:%20JavaCC%20Lexer%207.1%20Tutorial[+Send Us Your Feedback+]
