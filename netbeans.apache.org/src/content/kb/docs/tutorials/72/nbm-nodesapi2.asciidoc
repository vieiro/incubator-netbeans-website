...
[start=2]
2. When the wizard opens, name the class "EventNode" and press Enter or click Finish.
...
[start=6]
6. Now you will make a similar change to the Children object. Open the `EventChildFactory` class in the editor, and change its `createNodes` method as follows:
...
[start=4]
4. Now if you run the Event Manager, all of the Nodes will have the correct icon, as shown below:
...
[start=5]
5. Run the Event Manager again and notice that you now have the following:
...
[start=3]
3. Right click the EventManager and choose Run and then, once it is started up, use Window > Open Editor to show your editor.

[start=4]
4. Select Window > Properties to show the NetBeans property sheet.
...
[start=4]
4. Add the following line to the constructor which takes an argument of `Event`:
...
[start=6]
6. Run the module Event Manager again, select a `EventNode` in the `MyEditor` window and change its `Date` property—notice that the display name of the `Node` is now updated correctly, as shown below, where the year 2009 and is now reflected both on the node and in the property sheet:
...
link:https://netbeans.org/about/contact_form.html?to=3&subject=Feedback:%20Nodes%20API%207.1%20Module%20Tutoria[+Send Us Your Feedback+]
