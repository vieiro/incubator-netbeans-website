...
* Using the NetBeans link:https://netbeans.org/download/dev/javadoc/org-openide-nodes/overview-summary.html[+Nodes API+] to create nodes for feeds and feed entries.
...
[start=2]
2. Choose File > New Project (Ctrl-Shift-N), then expand Samples, then select NetBeans Modules, and choose FeedReader. Click Next.
