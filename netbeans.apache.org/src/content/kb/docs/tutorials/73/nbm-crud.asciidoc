...
Software or ResourceVersion Requiredlink:https://netbeans.org/downloads/index.html[+NetBeans IDE+]version 7.2 or 7.1link:http://java.sun.com/javase/downloads/index.jsp[+Java Developer Kit (JDK)+]version 6 or above
...
[start=2]
2. In the Favorites window (Ctrl-3), notice that you now have the following project structure:
...
[start=3]
3. Right-click the  ``CRUD Customer DB Manager``  project and choose Run:
...
[start=2]
2. In the "New Library Wrapper Module Project" dialog, select the JAR you created in the previous subsection. No need to include a license; leave the License field empty:
