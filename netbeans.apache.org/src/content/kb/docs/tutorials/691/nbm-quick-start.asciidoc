...
[start=6]
6. In the same way as shown in the previous step, set a dependency on the Lookup API module, which provides the ServiceProvider annotation that you will use in the next step.

[start=7]
7. Because of the dependency you defined above, you can now implement the interface defined in the second module:
...
[start=5]
5. Add code to the constructor of the HistoryTopComponent class so that it listens to the lookup of the  ``String``  class of the current active window. It displays all retrieved  ``String``  objects in the text area:*private Lookup.Result result;*
