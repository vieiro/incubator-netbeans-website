...
[start=2]
2. In the Name and Location panel, type  ``DemoAnnotation``  in the Project Name field. Change the Project Location to any directory on your computer. Leave the Standalone Module option and Set as Main Project checkbox selected. Click Next.

[start=3]
3. In the Basic Module Configuration panel, type  ``org.myorg.demoannotation``  in Code Name Base.

[start=4]
4. Select "Generate XML Layer". Leave the locations of both the localizing bundle and the XML layer file so that they will be stored in a package with the name  ``org/myorg/demoannotation`` . Click Finish.
...
* link:https://platform.netbeans.org/tutorials[+Other Related Tutorials+]
