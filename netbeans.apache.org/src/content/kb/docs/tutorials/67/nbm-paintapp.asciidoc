...
[start=8]
8. Fill out the methods from the  ``CallableSystemAction``  class as follows:
...
[start=9]
9. Fill out the  ``propertyChange()``  method from the  ``PropertyChangeListener``  as follows:
...
That's it! Your first complete application built on top of the NetBeans Platform is finished. Next stop: link:https://platform.netbeans.org/tutorials/nbm-feedreader.html[+NetBeans Platform Feed Reader Tutorial+]. 
