...
Após concluir este tutorial, você pode passar para a link:https://platform.netbeans.org/tutorials[+trilha do aprendizado da plataforma NetBeans+] A trilha do aprendizado fornece tutoriais abrangentes que destacam uma ampla gama de APIs do NetBeans para uma variedade de tipos de aplicativos. Se não desejar criar um aplicativo "Hello world", ignore este tutorial e vá direito para a trilha do aprendizado.
...
[start=2]
2. Altere a assinatura para que  ``CallableSystemAction``  estenda a classe, em vez de  ``ActionListener``  implementando-a. Adicione as sentenças import mostradas abaixo e, neste estágio, nada deve ser retornado para cada um dos métodos exigidos. A classe  ``GoogleAction``  agora deve ter esta aparência:
...
[start=4]
4. De volta à classe  ``GoogleAction`` , abaixo da declaração de classe, declare e defina a seguinte variável:
...
[start=5]
5. Novamente na classe  ``GoogleAction`` , defina o método  ``getToolbarPresenter()``  para retornar a variável  ``retValue`` :
