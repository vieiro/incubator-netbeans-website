...
[start=7]
7. ソースエディタで、 ``SaveCanvasAction.java``  が開いていることを確認し、 ``performAction()``  メソッドに次のように記述します。
...
以上でチュートリアルは終了です。NetBeans プラットフォーム上での最初のアプリケーションの構築が完了しました。次は、link:https://platform.netbeans.org/tutorials/nbm-feedreader.html[+NetBeans プラットフォームフィードリーダーのチュートリアル+]です。 
