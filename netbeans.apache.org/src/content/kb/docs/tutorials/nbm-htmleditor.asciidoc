...
For troubleshooting purposes, you are welcome to download the link:http://java.net/projects/nb-api-samples/sources/api-samples/show/versions/8.1/tutorials/HTMLEditor[+completed tutorial source code+].
...
[start=3]
3. We'll start by using the IDE to brand our application. Right-click the project node, choose Branding, and then make a few changes in the Branding Editor:
...
[start=6]
6. Also select the following in the  ``ide``  cluster:
